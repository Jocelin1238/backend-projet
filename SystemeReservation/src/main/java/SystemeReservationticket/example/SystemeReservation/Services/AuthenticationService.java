...
public class AuthenticationService {
    ...
    public AuthenticationReponse register(RegisterRequest request) {
        ...
        // 1. Vérifier si c'est le tout premier utilisateur de la base de données
        boolean isFirstUser = utilisateurRepository.count() == 0;
        
        // 2. Assigner ROLE_SUPER_ADMIN si c'est le premier, sinon ROLE_USER
        RolesTypes targetRoleType = isFirstUser ? RolesTypes.ROLE_SUPER_ADMIN : RolesTypes.ROLE_USER;

        Role roleUser = roleRepository.findByRole(targetRoleType)
                .orElseThrow(() -> new RuntimeException("Rôle " + targetRoleType + " introuvable."));
    ...
    public AuthenticationReponse createUserBySuperAdmin(RegisterRequest request) {
        ...
        Utilisateur utilisateur = Utilisateur.builder()
                ...
                .telephone(request.getTelephone())
                .adresse(request.getAdresse())
                .actif(true) // Créé directement actif par le super admin
                .compteExpire(false)
                .compteVerrouille(false)
                .credentialExpire(false)
                .roles(new HashSet<>())
